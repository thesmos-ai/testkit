...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.Sticky():     "MixedSuite.Checks.Model.Sticky()",
}
...
func (mixedModelChecks) Sticky() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.Sticky)
}

func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		mixedModelChecks{}.Agrees(),
		mixedModelChecks{}.Sticky(),
	}
}
...
type MixedCheck struct {
	...
	PropStore func(rt *PropT, s Mixed, value sticky.Value)
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		...
		{
			ID:    mixedCheckIndex.Model.Sticky(),
			Class: suite.ClassLaws,
			Claim: "once a key resolves, it keeps resolving to the same value",
			Binds: []string{
				lawid.Sticky,
			},
...
//	           crash recovery — the first write to a key pins it, so a later acknowledged write is one the store never promised to answer with
...
// testkit:provenance 19280520bdfed4983df0007867fcf9f59d8bb1cb4611e7f0f6b4c2164fd1c89c
