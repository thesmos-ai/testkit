...
//	model/mixed/AUTO-STREAM-OVER-MATCH
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.StreamOverMatch(): "MixedSuite.Checks.Model.StreamOverMatch()",
}
...
func (mixedModelChecks) StreamOverMatch() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.StreamOverMatch)
}
...
func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		...
		mixedModelChecks{}.StreamOverMatch(),
	}
}
...
func mixedProofs() prove.Defects[Mixed] {
	...
	return prove.Defects[Mixed]{
		...
		ix.Model.StreamOverMatch(): prove.One("a Mixed whose Items reports success and keeps nothing",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedItems(
					func(_ context.Context) (r0 []overmatch.Value, err error) {
						// The call arrives and nothing is done with it; the bare
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		...
		{
			ID:    mixedCheckIndex.Model.StreamOverMatch(),
			Class: suite.ClassLaws,
			Claim: "the drain holds everything that was written, and may hold more",
			Binds: []string{
				lawid.StreamOverMatch,
			},
			Falsifiable: suite.Proven(),
			Strength:    suite.StrengthDifferential,
			RunWith: func(tb testing.TB, sub suite.Subject[Mixed]) {
				mixedAssertStreamOverMatch(tb, sub, fx)
			},
		},
	}
}
...
type mixedModelReference struct {
	store *ref.Collection[overmatch.Value]
}
...
func NewMixedModelReference() Mixed {
	return &mixedModelReference{store: ref.NewCollection[overmatch.Value]()}
}
...
func mixedAssertAgrees(
	...
	appendHist := history.New[string, overmatch.Value]()
...
func mixedAssertStreamOverMatch(
	...
	legs.Law(tb, sub,
		...
		mixedModelActions(fx, appendHist),
		[]law.Law[Mixed]{
			law.StreamOverMatch[overmatch.Mixed, overmatch.Value, overmatch.Value]{
				Drain: func(rt *model.T, s overmatch.Mixed) ([]overmatch.Value, error) {
					return s.Items(rt.Context())
				},
				History: appendHist,
				Hash:    func(v overmatch.Value) overmatch.Value { return v },
			},
		},
		model.WithHistoryReset[Mixed](appendHist.Reset))
}
...
// testkit:provenance a45d74b068ffda90d66f52f0565bf8a89a52c272603559e2bee766ccbbb3d04c
