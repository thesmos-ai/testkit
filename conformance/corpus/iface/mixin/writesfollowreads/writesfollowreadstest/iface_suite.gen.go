...
//	model/mixed/AUTO-WRITES-FOLLOW-READS
//
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.WritesFollowReads(): "MixedSuite.Checks.Model.WritesFollowReads()",
}
...
func (mixedModelChecks) WritesFollowReads() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.WritesFollowReads)
}

func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		mixedModelChecks{}.Linearizable(),
		mixedModelChecks{}.WritesFollowReads(),
	}
}
...
type MixedCheck struct {
	...
	PropStore func(rt *PropT, s Mixed, value writesfollowreads.Value)
...
func mixedProofs() prove.Defects[Mixed] {
	...
	return prove.Defects[Mixed]{
		ix.Store.Smoke(): prove.One("a Mixed whose Store panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedStore(
					func(_ context.Context, _ writesfollowreads.Value) (writesfollowreads.Value, error) {
		...
		ix.Store.Cancels(): prove.One("a Mixed whose Store ignores the context it is handed",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedStore(
					func(_ context.Context, _ writesfollowreads.Value) (r0 writesfollowreads.Value, err error) {
			...
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedStore(
					func(_ context.Context, _ writesfollowreads.Value) (r0 writesfollowreads.Value, err error) {
						...
						r0 = writesfollowreads.Value{Key: "other-value"}
		...
		ix.Get.Smoke(): prove.One("a Mixed whose Get panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedGet(
					func(_ context.Context, _ string) (writesfollowreads.Value, error) {
		...
		ix.Get.Cancels(): prove.One("a Mixed whose Get ignores the context it is handed",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedGet(
					func(_ context.Context, _ string) (r0 writesfollowreads.Value, err error) {
						// The call arrives and nothing is done with it; the bare
						// return answers every slot's zero, which for the error
						// slot is the nil this claim forbids.
						return
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		...
		{
			ID:    mixedCheckIndex.Model.WritesFollowReads(),
			Class: suite.ClassLaws,
			Claim: "a client's write is stamped no older than what it has read",
			Binds: []string{
				lawid.WritesFollowReads,
			},
			Falsifiable: suite.Argued("no rule in this generator plants a defect for this claim — either nothing reaches it from shape and stamps alone, or nobody has written the rule; the defect is yours to write and this row claims no proof"),
			Strength:    suite.StrengthObserved,
			RunWith: func(tb testing.TB, sub suite.Subject[Mixed]) {
				mixedAssertWritesFollowReads(tb, sub, fx)
			},
		},
	}
}
...
// mixedAssertWritesFollowReads binds AUTO-WRITES-FOLLOW-READS over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func mixedAssertWritesFollowReads(
	tb testing.TB,
	sub suite.Subject[Mixed],
	fx MixedFixture,
) {
...
// testkit:provenance 62ba59469254afb4e00ed8b7f8bee5898aa90a5d9f0031b73a151635a3fcb3a3
