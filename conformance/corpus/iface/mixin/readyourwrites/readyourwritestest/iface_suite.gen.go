...
//	model/mixed/AUTO-READ-YOUR-WRITES
//
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.ReadYourWrites(): "MixedSuite.Checks.Model.ReadYourWrites()",
}
...
func (mixedModelChecks) ReadYourWrites() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.ReadYourWrites)
}

func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		mixedModelChecks{}.Linearizable(),
		mixedModelChecks{}.ReadYourWrites(),
	}
}
...
type MixedCheck struct {
	...
	PropStore func(rt *PropT, s Mixed, value readyourwrites.Value)
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		...
		{
			ID:    mixedCheckIndex.Model.ReadYourWrites(),
			Class: suite.ClassLaws,
			Claim: "a client reading after its own write sees that write or a later one",
			Binds: []string{
				lawid.ReadYourWrites,
			},
			Falsifiable: suite.Argued("no rule in this generator plants a defect for this claim — either nothing reaches it from shape and stamps alone, or nobody has written the rule; the defect is yours to write and this row claims no proof"),
			Strength:    suite.StrengthObserved,
			RunWith: func(tb testing.TB, sub suite.Subject[Mixed]) {
				mixedAssertReadYourWrites(tb, sub, fx)
			},
		},
	}
}
...
// mixedAssertReadYourWrites binds AUTO-READ-YOUR-WRITES over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func mixedAssertReadYourWrites(
	tb testing.TB,
	sub suite.Subject[Mixed],
	fx MixedFixture,
) {
...
// testkit:provenance 6f878cb2fd321cce8dd0be340f3495f623cc208066498bb9523651de14eb3723
