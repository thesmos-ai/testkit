...
type MixedCheck struct {
	...
	PropPut func(rt *PropT, s Mixed, value ttl.Value)
...
func mixedAssertLinearizable(
	...
	legs.Concurrent(tb, sub,
		...
		model.ConcurrentConfig[Mixed]{
			Model: linearize.KV[string, ttl.Value](ttl.ErrExpired),
...
func mixedAssertRecovery(
	...
	tb.Helper()
	legs.Recover[Mixed, string, ttl.Value](tb, sub,
		crash.Schedule[Mixed, string, ttl.Value]{
			Values: mixedModelValues(fx),
			Keys:   mixedModelKeys(fx),
			KeyOf:  mixedModelKeyOf,
			Write: func(ctx context.Context, world Mixed, v ttl.Value) error {
				return ttl.Mixed(world).Put(ctx, v)
			},
			Read: func(ctx context.Context, world Mixed, k string) (ttl.Value, error) {
				return ttl.Mixed(world).Read(ctx, k)
			},
			Equal:  func(a, b ttl.Value) bool { return a == b },
			Absent: crash.Is(ttl.ErrExpired),
		})
}
...
// testkit:provenance b577d2264549e8f1729aeee579e244c3db313209822c0a5dc7cd01d46f8f0fea
