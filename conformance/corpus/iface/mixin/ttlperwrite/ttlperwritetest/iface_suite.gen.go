...
func (f MixedFixture) Entry() ttlperwrite.Entry { return f.entry }
...
func (f MixedFixture) EntryOther() ttlperwrite.Entry { return f.entryOther }
...
func mixedSignatureChecks(fx MixedFixture) []suite.Check[Mixed] {
	...
	return []suite.Check[Mixed]{
		sig(ix.Put.Smoke(), suite.ClassSmoke,
			"Put survives a call with a derived entry",
...
// mixedAssertPutSmoke asserts Put survives a call with a derived entry.
...
func mixedAssertReadZeroOnError(
	...
	if err == nil {
		tb.Skipf("Read answered for an input nothing wrote, so this check has no error to inspect")
	}

	var zero ttlperwrite.Entry
	if got != zero {
...
func mixedAssertReadMiss(
	...
	if !errors.Is(err, ttlperwrite.ErrExpired) {
		tb.Errorf("Read must report %v: got %v", ttlperwrite.ErrExpired, err)
	}
}
...
type MixedCheck struct {
	...
	PropPut func(rt *PropT, s Mixed, value ttlperwrite.Entry)
...
func mixedProofs() prove.Defects[Mixed] {
	...
	return prove.Defects[Mixed]{
		ix.Put.Smoke(): prove.One("a Mixed whose Put panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedPut(
					func(_ context.Context, _ ttlperwrite.Entry) error {
		...
		ix.Put.Cancels(): prove.One("a Mixed whose Put ignores the context it is handed",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedPut(
					func(_ context.Context, _ ttlperwrite.Entry) (err error) {
		...
		ix.Read.Smoke(): prove.One("a Mixed whose Read panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedRead(
					func(_ context.Context, _ string) (ttlperwrite.Entry, error) {
...
//
//	Reference: derived — a keyed map over the Entry fixture pair,
...
//
// The fixture pair alone — time.Duration reaches a type this build cannot prove a wide draw serves.
func mixedModelValues(fx MixedFixture) *model.Generator[ttlperwrite.Entry] {
	keys := mixedModelKeys(fx)
	bodies := model.SampledFrom([]ttlperwrite.Entry{fx.Entry(), fx.EntryOther()})
	...
	return model.Custom(func(t *model.T) ttlperwrite.Entry {
...
func mixedModelKeyOf(v ttlperwrite.Entry) string {
	return v.Key
}
...
type mixedModelReference struct {
	store *ref.MapStore[string, ttlperwrite.Entry]
}
...
func NewMixedModelReference() Mixed {
	return &mixedModelReference{store: ref.NewMapStore[string, ttlperwrite.Entry](
		mixedModelKeyOf,
		ttlperwrite.ErrExpired,
	)}
}

func (r *mixedModelReference) Put(ctx context.Context, e ttlperwrite.Entry) error {
	return r.store.Put(ctx, e)
}

func (r *mixedModelReference) Read(ctx context.Context, key string) (ttlperwrite.Entry, error) {
...
func mixedModelActions(fx MixedFixture) []model.Action[Mixed] {
	...
	return []model.Action[Mixed]{
		action.Writer("Put", values,
			func(ctx context.Context, s ttlperwrite.Mixed, v ttlperwrite.Entry) error {
				return s.Put(ctx, v)
			}),
		action.Reader("Read", keys,
			func(ctx context.Context, s ttlperwrite.Mixed, k string) (ttlperwrite.Entry, error) {
				return s.Read(ctx, k)
			}, action.WithSentinel(ttlperwrite.ErrExpired)),
	}
}
...
func mixedAssertLinearizable(
	...
	legs.Concurrent(tb, sub,
		...
		model.ConcurrentConfig[Mixed]{
			Model: linearize.KV[string, ttlperwrite.Entry](ttlperwrite.ErrExpired),
...
func mixedAssertRecovery(
	...
	tb.Helper()
	legs.Recover[Mixed, string, ttlperwrite.Entry](tb, sub,
		crash.Schedule[Mixed, string, ttlperwrite.Entry]{
			Values: mixedModelValues(fx),
			Keys:   mixedModelKeys(fx),
			KeyOf:  mixedModelKeyOf,
			Write: func(ctx context.Context, world Mixed, v ttlperwrite.Entry) error {
				return ttlperwrite.Mixed(world).Put(ctx, v)
			},
			Read: func(ctx context.Context, world Mixed, k string) (ttlperwrite.Entry, error) {
				return ttlperwrite.Mixed(world).Read(ctx, k)
			},
			Equal:  func(a, b ttlperwrite.Entry) bool { return a == b },
			Absent: crash.Is(ttlperwrite.ErrExpired),
		})
}
...
func mixedAssertWriteObservable(
	...
	legs.Law(tb, sub,
		...
		mixedModelActions(fx),
		[]law.Law[Mixed]{
			law.WriteObservable[ttlperwrite.Mixed, ttlperwrite.Entry, string]{
				Write: func(rt *model.T, s ttlperwrite.Mixed, v ttlperwrite.Entry) error {
...
// testkit:provenance f77a82da9055dde3db3a5c046d847ffda895cbe8a20da0b982577f2405de2bdd
