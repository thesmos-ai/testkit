...
//	Store/nilcontext
//	Store/smoke
//	model/mixed/AUTO-DEFAULT-ON-ERROR
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.DefaultOnError():  "MixedSuite.Checks.Model.DefaultOnError()",
...
func (mixedModelChecks) DefaultOnError() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.DefaultOnError)
}
...
func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		...
		mixedModelChecks{}.DefaultOnError(),
	}
}
...
type MixedCheck struct {
	...
	PropStore func(rt *PropT, s Mixed, value defaultonerror.Value)
...
func mixedProofs() prove.Defects[Mixed] {
	...
	return prove.Defects[Mixed]{
		ix.Store.Smoke(): prove.One("a Mixed whose Store panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedStore(
					func(_ context.Context, _ defaultonerror.Value) error {
		...
		ix.Store.Cancels(): prove.One("a Mixed whose Store ignores the context it is handed",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedStore(
					func(_ context.Context, _ defaultonerror.Value) (err error) {
		...
		ix.Get.Smoke(): prove.One("a Mixed whose Get panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedGet(
					func(_ context.Context, _ string) (defaultonerror.Value, error) {
		...
		ix.Get.Cancels(): prove.One("a Mixed whose Get ignores the context it is handed",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedGet(
					func(_ context.Context, _ string) (r0 defaultonerror.Value, err error) {
						// The call arrives and nothing is done with it; the bare
						// return answers every slot's zero, which for the error
						// slot is the nil this claim forbids.
						return
			...
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedGet(
					func(_ context.Context, _ string) (r0 defaultonerror.Value, err error) {
						...
						r0 = defaultonerror.Value{Key: "other-value"}
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		...
		{
			ID:    mixedCheckIndex.Model.DefaultOnError(),
			Class: suite.ClassLaws,
			Claim: "a read that errs answers the zero value, for every key the run draws",
			Binds: []string{
				lawid.DefaultOnError,
			},
			Falsifiable: suite.Argued("no rule in this generator plants a defect for this claim — either nothing reaches it from shape and stamps alone, or nobody has written the rule; the defect is yours to write and this row claims no proof"),
			Strength:    suite.StrengthDifferential,
			RunWith: func(tb testing.TB, sub suite.Subject[Mixed]) {
				mixedAssertDefaultOnError(tb, sub, fx)
			},
		},
	}
}
...
func mixedAssertLinearizable(
	...
	legs.Concurrent(tb, sub,
		...
		model.ConcurrentConfig[Mixed]{
			Model: linearize.KV[string, defaultonerror.Value](nil),
...
func mixedAssertRecovery(
	...
	tb.Helper()
	legs.Recover[Mixed, string, defaultonerror.Value](tb, sub,
		crash.Schedule[Mixed, string, defaultonerror.Value]{
			Values: mixedModelValues(fx),
			Keys:   mixedModelKeys(fx),
			KeyOf:  mixedModelKeyOf,
			Write: func(ctx context.Context, world Mixed, v defaultonerror.Value) error {
				return defaultonerror.Mixed(world).Store(ctx, v)
			},
			Read: func(ctx context.Context, world Mixed, k string) (defaultonerror.Value, error) {
				return defaultonerror.Mixed(world).Get(ctx, k)
			},
			Equal:  func(a, b defaultonerror.Value) bool { return a == b },
			Absent: nil,
		})
}
...
// mixedAssertDefaultOnError binds AUTO-DEFAULT-ON-ERROR over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func mixedAssertDefaultOnError(
	tb testing.TB,
	sub suite.Subject[Mixed],
	fx MixedFixture,
) {
...
// testkit:provenance bdc5ae4dda4b25abd03de165464fda1f5b4e488dc136ac5a2427640204c9446f
