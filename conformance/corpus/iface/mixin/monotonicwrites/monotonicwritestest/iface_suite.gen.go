...
//	model/mixed/AUTO-MONOTONIC-WRITES
//
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.MonotonicWrites(): "MixedSuite.Checks.Model.MonotonicWrites()",
}
...
func (mixedModelChecks) MonotonicWrites() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.MonotonicWrites)
}

func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		mixedModelChecks{}.Linearizable(),
		mixedModelChecks{}.MonotonicWrites(),
	}
}
...
type MixedCheck struct {
	...
	PropStore func(rt *PropT, s Mixed, value monotonicwrites.Value)
		...
		ix.Store.Smoke(): prove.One("a Mixed whose Store panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedStore(
					func(_ context.Context, _ monotonicwrites.Value) (monotonicwrites.Value, error) {
		...
		ix.Get.Smoke(): prove.One("a Mixed whose Get panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedGet(
					func(_ context.Context, _ string) (monotonicwrites.Value, error) {
	...
	return []suite.Check[Mixed]{
		{
			...
			Claim: "the session guarantees this interface declares hold when calls overlap",
			Binds: []string{
				lawid.MonotonicWrites,
			},
		...
		{
			ID:    mixedCheckIndex.Model.MonotonicWrites(),
			Class: suite.ClassLaws,
			Claim: "a client's successive writes to a key are stamped in issue order",
			Binds: []string{
				lawid.MonotonicWrites,
			},
			Falsifiable: suite.Argued("no rule in this generator plants a defect for this claim — either nothing reaches it from shape and stamps alone, or nobody has written the rule; the defect is yours to write and this row claims no proof"),
			Strength:    suite.StrengthObserved,
			RunWith: func(tb testing.TB, sub suite.Subject[Mixed]) {
				mixedAssertMonotonicWrites(tb, sub, fx)
			},
		},
	}
}
...
// mixedAssertMonotonicWrites binds AUTO-MONOTONIC-WRITES over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func mixedAssertMonotonicWrites(
	tb testing.TB,
	sub suite.Subject[Mixed],
	fx MixedFixture,
) {
...
// testkit:provenance 35daba0cab02012013dd7cf7995fc1bfa8caa6e60f45e05d439ebd9c253ba2da
