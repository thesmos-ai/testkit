...
//	model/mixed/AUTO-STREAM-NO-DUPLICATES
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.StreamNoDuplicates(): "MixedSuite.Checks.Model.StreamNoDuplicates()",
}
...
func (mixedModelChecks) StreamNoDuplicates() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.StreamNoDuplicates)
}
...
func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		...
		mixedModelChecks{}.StreamNoDuplicates(),
	}
}
...
type MixedCheck struct {
	...
	PropAdd func(rt *PropT, s Mixed, value noduplicates.Value)
}
		...
		ix.Add.Smoke(): prove.One("a Mixed whose Add panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedAdd(
					func(_ context.Context, _ noduplicates.Value) error {
		...
		ix.Items.Smoke(): prove.One("a Mixed whose Items panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedItems(
					func(_ context.Context) ([]noduplicates.Value, error) {
			...
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedItems(
					func(_ context.Context) (r0 []noduplicates.Value, err error) {
						...
						r0 = []noduplicates.Value{{Key: "other-"}}
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		...
		{
			ID:    mixedCheckIndex.Model.StreamNoDuplicates(),
			Class: suite.ClassLaws,
			Claim: "each drain emits every element at most once",
			Binds: []string{
				lawid.StreamNoDuplicates,
			},
			...
			Strength:    suite.StrengthDifferential,
			RunWith: func(tb testing.TB, sub suite.Subject[Mixed]) {
				mixedAssertStreamNoDuplicates(tb, sub, fx)
			},
		},
	}
}
...
// mixedAssertStreamNoDuplicates binds AUTO-STREAM-NO-DUPLICATES over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func mixedAssertStreamNoDuplicates(
	tb testing.TB,
	sub suite.Subject[Mixed],
	fx MixedFixture,
) {
...
// testkit:provenance 9794b8e6cb7cea2f8abc4057a8b0c9fc332a12cd21e25c9ee630ba6c9bfe494b
