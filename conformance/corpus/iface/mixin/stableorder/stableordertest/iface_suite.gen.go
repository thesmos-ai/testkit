...
//	model/mixed/AUTO-STREAM-STABLE-ORDER
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.StreamStableOrder(): "MixedSuite.Checks.Model.StreamStableOrder()",
}
...
func (mixedModelChecks) StreamStableOrder() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.StreamStableOrder)
}
...
func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		...
		mixedModelChecks{}.StreamStableOrder(),
	}
}
...
type MixedCheck struct {
	...
	PropAdd func(rt *PropT, s Mixed, value stableorder.Value)
}
...
func mixedProofs() prove.Defects[Mixed] {
	...
	return prove.Defects[Mixed]{
		ix.Add.Smoke(): prove.One("a Mixed whose Add panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedAdd(
					func(_ context.Context, _ stableorder.Value) error {
		...
		ix.Add.Cancels(): prove.One("a Mixed whose Add ignores the context it is handed",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedAdd(
					func(_ context.Context, _ stableorder.Value) (err error) {
		...
		ix.Items.Smoke(): prove.One("a Mixed whose Items panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedItems(
					func(_ context.Context) ([]stableorder.Value, error) {
		...
		ix.Items.Cancels(): prove.One("a Mixed whose Items ignores the context it is handed",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedItems(
					func(_ context.Context) (r0 []stableorder.Value, err error) {
						// The call arrives and nothing is done with it; the bare
						// return answers every slot's zero, which for the error
						// slot is the nil this claim forbids.
						return
			...
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedItems(
					func(_ context.Context) (r0 []stableorder.Value, err error) {
						...
						r0 = []stableorder.Value{{Key: "other-"}}
		...
		ix.Model.Counts(): prove.One("a Mixed whose Items answers zero whatever is held",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedItems(
					func(_ context.Context) (r0 []stableorder.Value, err error) {
						// The call arrives and nothing is done with it; the bare
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		...
		{
			ID:    mixedCheckIndex.Model.StreamStableOrder(),
			Class: suite.ClassLaws,
			Claim: "the drain's order follows the declared one",
			Binds: []string{
				lawid.StreamStableOrder,
			},
			Needs: suite.Caps{
				"less": nil,
			},
			...
			Strength:    suite.StrengthDifferential,
			RunWith: func(tb testing.TB, sub suite.Subject[Mixed]) {
				mixedAssertStreamStableOrder(tb, sub, fx)
			},
		},
	}
}
...
// mixedAssertStreamStableOrder binds AUTO-STREAM-STABLE-ORDER over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func mixedAssertStreamStableOrder(
	tb testing.TB,
	sub suite.Subject[Mixed],
	fx MixedFixture,
) {
	...
	legs.Law(tb, sub,
		...
		mixedModelActions(fx),
		[]law.Law[Mixed]{
			law.StreamStableOrder[stableorder.Mixed, stableorder.Value]{
				Drain: func(rt *model.T, s stableorder.Mixed) ([]stableorder.Value, error) {
					return s.Items(rt.Context())
				},
				Less: suite.Provided[func(a, b stableorder.Value) bool](tb, sub, "less"),
			},
		})
}
...
// testkit:provenance 02056e5e094479594e19a1f584967f8825a26ad477264f8f84f2ad26279b51a4
