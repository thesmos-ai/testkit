...
//	Read/smoke
//	Read/zero-on-error
//	model/mixed/AUTO-IDEMPOTENT-WRITE
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.IdempotentWrite(): "MixedSuite.Checks.Model.IdempotentWrite()",
}
...
func (mixedModelChecks) IdempotentWrite() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.IdempotentWrite)
}
...
func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		...
		mixedModelChecks{}.IdempotentWrite(),
	}
}
...
func mixedProofs() prove.Defects[Mixed] {
	...
	return prove.Defects[Mixed]{
		...
		ix.Model.IdempotentWrite(): prove.One("a Mixed whose Put refuses its repeat",
			func(tb testing.TB) Mixed {
				called := false
				return NewMixedStub(tb, WithMixedPut(
					func(_ context.Context, _ string, _ string) (err error) {
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		...
		{
			ID:    mixedCheckIndex.Model.IdempotentWrite(),
			Class: suite.ClassLaws,
			...
			Strength:    suite.StrengthDifferential,
			RunWith: func(tb testing.TB, sub suite.Subject[Mixed]) {
				mixedAssertIdempotentWrite(tb, sub, fx)
			},
		},
	}
}
