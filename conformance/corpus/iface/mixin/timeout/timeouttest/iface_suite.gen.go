...
//	Slow/nilcontext
//	Slow/smoke
//	model/mixed/AUTO-DEADLINE-RESPECTING
//
...
var mixedIndexPath = map[suite.ID]string{
	...
	mixedCheckIndex.Model.Deadline():  "MixedSuite.Checks.Model.Deadline()",
}
...
func (mixedModelChecks) Deadline() suite.ID {
	return suite.FamilyID(suite.FamilyModel, mixedQualifier, lawid.DeadlineRespecting)
}

func (mixedModelChecks) All() []suite.ID {
	return []suite.ID{
		mixedModelChecks{}.Deadline(),
	}
}
...
type MixedCheck struct {
	...
	// Slow/<Name> in output and reruns with the rest of that
	...
	// PropSlow is Prop with Slow's own argument already drawn
	...
	// Slow, so leave Method empty.
	PropSlow func(rt *PropT, s Mixed, value string)
}
...
var mixedMethods = suite.NewNameSet("Mixed", mixedSlow)
...
func (c MixedCheck) bind(
	...
	if c.PropSlow != nil {
		bodies++
		out.ID = suite.MethodID(mixedSlow, c.Name)
		fn := c.PropSlow
		out.RunWith = func(tb testing.TB, sub suite.Subject[Mixed]) {
			model.Check(tb, func(rt *PropT) {
...
//		MixedSuite.Without(MixedSuite.Checks.Slow.Smoke()),
//	)
//
	...
	return prove.Defects[Mixed]{
		ix.Slow.Smoke(): prove.One("a Mixed whose Slow panics",
			func(tb testing.TB) Mixed {
				return NewMixedStub(tb, WithMixedSlow(
...
func mixedModelRows(fx MixedFixture) []suite.Check[Mixed] {
	return []suite.Check[Mixed]{
		{
			ID:    mixedCheckIndex.Model.Deadline(),
			Class: suite.ClassClocked,
			Claim: "an operation given a deadline returns once that deadline fires",
			Binds: []string{
...
// mixedAssertDeadline binds AUTO-DEADLINE-RESPECTING over the shared sequences, on a clock
// the run controls.
//
...
func mixedAssertDeadline(
	...
	legs.Law(tb, sub,
		...
		[]law.Law[Mixed]{
			timeaware.DeadlineRespecting[timeout.Mixed]{
...
// testkit:provenance cce6b9253d1d363b5bf76daa961c0e696689427f8ef40007b8a8ec0f8909b75d
