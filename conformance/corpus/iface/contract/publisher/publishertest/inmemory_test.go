...
func TestContractContract(t *testing.T) {
	...
	publishertest.RunContract(t, inMemory("in-memory"), contractChecks)
}
...
func TestContractContractWithoutSmoke(t *testing.T) {
	...
	publishertest.RunContract(t,
		inMemory("in-memory"),
		publishertest.ContractSuite.Without(
			publishertest.ContractSuite.Checks.Publish.Smoke(),
		),
	)
}
...
func TestContractChecksCanFail(t *testing.T) {
	...
	publishertest.ProveContract(t, contractChecks)
}
...
func inMemory(
	name string,
) publishertest.ContractHarness[*publishertest.InMemory] {
	return publishertest.ContractHarness[*publishertest.InMemory]{
		Name: name, New: publishertest.NewInMemory,
	}
}

// --- The checks: claims, bodies and defects, by name --------------------------

var contractChecks = publishertest.ContractChecks{
	{
		...
		ProvenBy: publishertest.BrokenContract(
...
func receivesWhatIsPublishedAfter(
	tb testing.TB, s publisher.Contract,
	fx publishertest.ContractFixture,
) {
	tb.Helper()
...
type plantedPublisher struct {
	wrong fault
	mu    sync.Mutex
	subs  []chan publisher.Value
}

func (p *plantedPublisher) Subscribe(
	context.Context,
) (<-chan publisher.Value, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	ch := make(chan publisher.Value, plantedBuffer)
...
func (p *plantedPublisher) Publish(
	_ context.Context, v publisher.Value,
) error {
...
func TestContractLawsCanSaturate(t *testing.T) {
	...
	publishertest.ContractModelSaturation(t, func() publishertest.Contract { return publishertest.NewInMemory() })
}
