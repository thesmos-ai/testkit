...
//	Run/smoke
//	Run/zero-on-error
//	model/contract/AUTO-APPENDER-MONOTONIC-OFFSETS
...
//
//	Run's miss check — no method here stores anything and nothing loads it with starting data, so there is no input this check could ask for and be sure was never stored. To close it: say what a lookup reports when it finds nothing, with //testkit:mixin notfound sentinel=Err…, or write the check yourself.
//
...
func contractNewFixture() ContractFixture {
	return ContractFixture{
		value:      appender.Value{Key: "test-key", Body: "test-body"},
		valueOther: appender.Value{Key: "other-key", Body: "other-body"},
	}
}

// Value is the value the checks call your implementation with.
func (f ContractFixture) Value() appender.Value { return f.value }
...
func (f ContractFixture) ValueOther() appender.Value { return f.valueOther }
...
type Contract = appender.Contract
...
var contractIndexPath = map[suite.ID]string{
	...
	contractCheckIndex.Model.MonotonicOffsets(): "ContractSuite.Checks.Model.MonotonicOffsets()",
}
...
func (contractModelChecks) MonotonicOffsets() suite.ID {
	return suite.FamilyID(suite.FamilyModel, contractQualifier, lawid.AppenderMonotonicOffsets)
}

func (contractModelChecks) All() []suite.ID {
	return []suite.ID{
		contractModelChecks{}.Linearizable(),
		contractModelChecks{}.MonotonicOffsets(),
	}
}
...
func contractSignatureChecks(fx ContractFixture) []suite.Check[Contract] {
	...
	return []suite.Check[Contract]{
		sig(ix.Run.Smoke(), suite.ClassSmoke,
			"Run survives a call with a seeded value",
...
// contractAssertRunSmoke asserts Run survives a call with a seeded value.
...
type ContractCheck struct {
	...
	PropRun func(rt *PropT, s Contract, key appender.Value)
}
...
func contractModelRows(fx ContractFixture) []suite.Check[Contract] {
	return []suite.Check[Contract]{
		...
		{
			ID:    contractCheckIndex.Model.MonotonicOffsets(),
			Class: suite.ClassAppender,
			Claim: "offsets of successive appends strictly increase",
			Binds: []string{
			...
			Strength:    suite.StrengthObserved,
			RunWith: func(tb testing.TB, sub suite.Subject[Contract]) {
				contractAssertMonotonicOffsets(tb, sub, fx)
			},
		},
	}
}
		...
		model.ConcurrentConfig[Contract]{
			...
			Actions: []model.ConcurrentAction[Contract]{
				linearize.ConcurrentAppend(linearize.OpAppend, keys,
					func(ctx context.Context, s appender.Contract, e appender.Value) (int64, error) {
						return s.Run(ctx, e)
					}),
			},
		})
...
// contractAssertMonotonicOffsets binds AUTO-APPENDER-MONOTONIC-OFFSETS over the shared sequences.
//
...
func contractAssertMonotonicOffsets(
	tb testing.TB,
	sub suite.Subject[Contract],
	fx ContractFixture,
) {
	...
	legs.Law(tb, sub,
		...
		[]law.Law[Contract]{
			&law.AppenderMonotonicOffsets[appender.Contract, appender.Value, int64]{
...
// testkit:provenance efa1b1c4e6c8ae3f1aa0bcd2eb0b4a61f3fdb6314bc99d31191555dc7cf8bdba
