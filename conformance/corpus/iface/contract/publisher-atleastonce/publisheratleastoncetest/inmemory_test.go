...
func TestContractContract(t *testing.T) {
	...
	publisheratleastoncetest.RunContract(t, inMemory("in-memory"), contractChecks)
}
...
func TestContractContractWithoutSmoke(t *testing.T) {
	...
	publisheratleastoncetest.RunContract(t,
		inMemory("in-memory"),
		publisheratleastoncetest.ContractSuite.Without(
			publisheratleastoncetest.ContractSuite.Checks.Publish.Smoke(),
		),
	)
}
...
func TestContractChecksCanFail(t *testing.T) {
	...
	publisheratleastoncetest.ProveContract(t, contractChecks)
}
...
func inMemory(
	...
	return publisheratleastoncetest.ContractHarness[*publisheratleastoncetest.InMemory]{
		Name: name, New: publisheratleastoncetest.NewInMemory,
	}
}

// --- The checks: claims, bodies and defects, by name --------------------------

var contractChecks = publisheratleastoncetest.ContractChecks{
	{
		...
		ProvenBy: publisheratleastoncetest.BrokenContract(
...
func receivesWhatIsPublishedAfter(
	tb testing.TB, s publisheratleastonce.Contract,
	fx publisheratleastoncetest.ContractFixture,
) {
	tb.Helper()
...
type plantedPublisher struct {
	wrong fault
	mu    sync.Mutex
	subs  []chan publisheratleastonce.Value
}

func (p *plantedPublisher) Subscribe(
	context.Context,
) (<-chan publisheratleastonce.Value, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	ch := make(chan publisheratleastonce.Value, plantedBuffer)
...
func (p *plantedPublisher) Publish(
	_ context.Context, v publisheratleastonce.Value,
) error {
	if p.wrong == publishesTheZero {
		v = publisheratleastonce.Value{}
	}
	p.mu.Lock()
	defer p.mu.Unlock()
...
func TestContractLawsCanSaturate(t *testing.T) {
	...
	publisheratleastoncetest.ContractModelSaturation(t, func() publisheratleastoncetest.Contract { return publisheratleastoncetest.NewInMemory() })
}
