...
//	model/contract/AUTO-UPSERTER-IDEMPOTENT
...
var contractIndexPath = map[suite.ID]string{
	...
	contractCheckIndex.Model.UpserterIdempotent(): "ContractSuite.Checks.Model.UpserterIdempotent()",
...
func (contractModelChecks) UpserterIdempotent() suite.ID {
	return suite.FamilyID(suite.FamilyModel, contractQualifier, lawid.UpserterIdempotent)
}
...
func (contractModelChecks) All() []suite.ID {
	return []suite.ID{
		...
		contractModelChecks{}.UpserterIdempotent(),
	}
}
...
type ContractCheck struct {
	...
	PropPut func(rt *PropT, s Contract, value upserter.Value)
...
func contractProofs() prove.Defects[Contract] {
	...
	return prove.Defects[Contract]{
		...
		ix.Model.UpserterIdempotent(): prove.One("a Contract whose Put refuses its repeat",
			func(tb testing.TB) Contract {
				called := false
				return NewContractStub(tb, WithContractPut(
					func(_ context.Context, _ upserter.Value) (err error) {
...
func contractModelRows(fx ContractFixture) []suite.Check[Contract] {
	return []suite.Check[Contract]{
		...
		{
			ID:    contractCheckIndex.Model.UpserterIdempotent(),
			Class: suite.ClassLaws,
			Claim: "upserting the same value twice leaves what the first upsert left",
			Binds: []string{
				lawid.UpserterIdempotent,
			},
			Falsifiable: suite.Proven(),
			Strength:    suite.StrengthDifferential,
			RunWith: func(tb testing.TB, sub suite.Subject[Contract]) {
				contractAssertUpserterIdempotent(tb, sub, fx)
			},
		},
	}
}
...
func contractAssertRecovery(
	...
	tb.Helper()
	legs.Recover[Contract, string, upserter.Value](tb, sub,
		crash.Schedule[Contract, string, upserter.Value]{
			Values: contractModelValues(fx),
			Keys:   contractModelKeys(fx),
			KeyOf:  contractModelKeyOf,
			Write: func(ctx context.Context, world Contract, v upserter.Value) error {
				return upserter.Contract(world).Put(ctx, v)
			},
			Read: func(ctx context.Context, world Contract, k string) (upserter.Value, error) {
				return upserter.Contract(world).Get(ctx, k)
			},
			Equal:  func(a, b upserter.Value) bool { return a == b },
			Absent: nil,
		})
}
...
// contractAssertUpserterIdempotent binds AUTO-UPSERTER-IDEMPOTENT over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func contractAssertUpserterIdempotent(
	tb testing.TB,
	sub suite.Subject[Contract],
	fx ContractFixture,
) {
...
// testkit:provenance c922105006a013eef6edadd3c4d04afc74451bade8af32e563a4080a00cf9bd3
