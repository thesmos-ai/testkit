...
var contractIndexPath = map[suite.ID]string{
	...
	contractCheckIndex.Model.AtomicWrite():     "ContractSuite.Checks.Model.AtomicWrite()",
...
func (contractModelChecks) AtomicWrite() suite.ID {
	return suite.FamilyID(suite.FamilyModel, contractQualifier, lawid.AtomicWrite)
}
...
func (contractModelChecks) All() []suite.ID {
	return []suite.ID{
		...
		contractModelChecks{}.AtomicWrite(),
	}
}
...
type ContractCheck struct {
	...
	PropPut func(rt *PropT, s Contract, value batchwriter.Value)
...
func contractModelRows(fx ContractFixture) []suite.Check[Contract] {
	return []suite.Check[Contract]{
		...
		{
			ID:    contractCheckIndex.Model.AtomicWrite(),
			Class: suite.ClassLaws,
			Claim: "a write that errs leaves the observable state unchanged",
			Binds: []string{
				lawid.AtomicWrite,
			},
			Falsifiable: suite.Argued("no rule in this generator plants a defect for this claim — either nothing reaches it from shape and stamps alone, or nobody has written the rule; the defect is yours to write and this row claims no proof"),
			Strength:    suite.StrengthDifferential,
			RunWith: func(tb testing.TB, sub suite.Subject[Contract]) {
				contractAssertAtomicWrite(tb, sub, fx)
			},
		},
	}
}
...
func contractAssertLinearizable(
	...
	legs.Concurrent(tb, sub,
		...
		model.ConcurrentConfig[Contract]{
			Model: linearize.KV[string, batchwriter.Value](nil),
...
func contractAssertRecovery(
	...
	tb.Helper()
	legs.Recover[Contract, string, batchwriter.Value](tb, sub,
		crash.Schedule[Contract, string, batchwriter.Value]{
			Values: contractModelValues(fx),
			Keys:   contractModelKeys(fx),
			KeyOf:  contractModelKeyOf,
			Write: func(ctx context.Context, world Contract, v batchwriter.Value) error {
				return batchwriter.Contract(world).Put(ctx, v)
			},
			Read: func(ctx context.Context, world Contract, k string) (batchwriter.Value, error) {
				return batchwriter.Contract(world).Get(ctx, k)
			},
...
// contractAssertAtomicWrite binds AUTO-ATOMIC-WRITE over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func contractAssertAtomicWrite(
	tb testing.TB,
	sub suite.Subject[Contract],
	fx ContractFixture,
) {
