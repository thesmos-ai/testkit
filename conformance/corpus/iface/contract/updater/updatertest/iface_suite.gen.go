...
//	model/contract/AUTO-UPDATER-REPLACES
...
var contractIndexPath = map[suite.ID]string{
	...
	contractCheckIndex.Model.UpdaterReplaces(): "ContractSuite.Checks.Model.UpdaterReplaces()",
...
func (contractModelChecks) UpdaterReplaces() suite.ID {
	return suite.FamilyID(suite.FamilyModel, contractQualifier, lawid.UpdaterReplaces)
}
...
func (contractModelChecks) All() []suite.ID {
	return []suite.ID{
		...
		contractModelChecks{}.UpdaterReplaces(),
	}
}
...
type ContractCheck struct {
	...
	PropPut func(rt *PropT, s Contract, value updater.Value)
...
func contractProofs() prove.Defects[Contract] {
	...
	return prove.Defects[Contract]{
		ix.Put.Smoke(): prove.One("a Contract whose Put panics",
			func(tb testing.TB) Contract {
				return NewContractStub(tb, WithContractPut(
					func(_ context.Context, _ updater.Value) error {
		...
		ix.Put.Cancels(): prove.One("a Contract whose Put ignores the context it is handed",
			func(tb testing.TB) Contract {
				return NewContractStub(tb, WithContractPut(
					func(_ context.Context, _ updater.Value) (err error) {
		...
		ix.Get.Smoke(): prove.One("a Contract whose Get panics",
			func(tb testing.TB) Contract {
				return NewContractStub(tb, WithContractGet(
					func(_ context.Context, _ string) (updater.Value, error) {
		...
		ix.Get.Cancels(): prove.One("a Contract whose Get ignores the context it is handed",
			func(tb testing.TB) Contract {
				return NewContractStub(tb, WithContractGet(
					func(_ context.Context, _ string) (r0 updater.Value, err error) {
						// The call arrives and nothing is done with it; the bare
						// return answers every slot's zero, which for the error
						// slot is the nil this claim forbids.
						return
			...
			func(tb testing.TB) Contract {
				return NewContractStub(tb, WithContractGet(
					func(_ context.Context, _ string) (r0 updater.Value, err error) {
						...
						r0 = updater.Value{Key: "other-value"}
...
func contractModelRows(fx ContractFixture) []suite.Check[Contract] {
	return []suite.Check[Contract]{
		...
		{
			ID:    contractCheckIndex.Model.UpdaterReplaces(),
			Class: suite.ClassLaws,
			Claim: "the second update of a key is the one that reads back",
			Binds: []string{
				lawid.UpdaterReplaces,
			},
			Falsifiable: suite.Argued("no rule in this generator plants a defect for this claim — either nothing reaches it from shape and stamps alone, or nobody has written the rule; the defect is yours to write and this row claims no proof"),
			Strength:    suite.StrengthDifferential,
			RunWith: func(tb testing.TB, sub suite.Subject[Contract]) {
				contractAssertUpdaterReplaces(tb, sub, fx)
			},
		},
	}
}
...
func contractAssertRecovery(
	...
	tb.Helper()
	legs.Recover[Contract, string, updater.Value](tb, sub,
		crash.Schedule[Contract, string, updater.Value]{
			Values: contractModelValues(fx),
			Keys:   contractModelKeys(fx),
			KeyOf:  contractModelKeyOf,
			Write: func(ctx context.Context, world Contract, v updater.Value) error {
				return updater.Contract(world).Put(ctx, v)
			},
			Read: func(ctx context.Context, world Contract, k string) (updater.Value, error) {
				return updater.Contract(world).Get(ctx, k)
			},
			Equal:  func(a, b updater.Value) bool { return a == b },
			Absent: nil,
		})
}
...
// contractAssertUpdaterReplaces binds AUTO-UPDATER-REPLACES over the shared sequences.
//
// One law, and the run's only oracle — see [legs.Law]
// for why the differential is off on every law leg.
func contractAssertUpdaterReplaces(
	tb testing.TB,
	sub suite.Subject[Contract],
	fx ContractFixture,
) {
...
// testkit:provenance f36eaaa248c200584210f67919a17d1c22e0986493b8d101c1d22f474acf6225
